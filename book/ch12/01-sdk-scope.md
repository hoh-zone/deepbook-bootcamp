...
- 限定 SDK 章的边界：构造 PTB、管理配置、dry run、错误解析，而不是托管用户私钥。
- 区分 DeepBook Spot/Margin 的 SDK 封装与 Predict 当前 raw Move 入口封装。
- 把 Predict package/object IDs、网络和版本状态作为服务初始化的必填项。
