...
- 看清 Margin 不是替代现货撮合，而是在 DeepBook Pool 外增加持续借贷、抵押和风险控制层。
- 能够沿着抵押、借款、下单、结算、还款和清算六个动作追踪资金从用户钱包到 `MarginManager`、`MarginPool` 和 DeepBook Pool 的变化。
- 能明确区分 Margin 借贷的 borrow shares 持续债务与 DeepBookV3 闪电贷的同交易原子借还。
