...
- 明确 DeepBookV3 闪电贷入口 属于 DeepBookV3 Spot pool 闪电贷，入口固定在 `pool.move`，底层固定在 `vault/vault.move`。
...
Pool 入口只负责加载 `PoolInner`，把 pool ID、数量和 coin 转交给 vault。真实资产在 `Vault` 的 `base_balance` 和 `quote_balance` 中 split 和 join。
