...
- 明确 失败闪电贷练习 属于 DeepBookV3 Spot pool 闪电贷，入口固定在 `pool.move`，底层固定在 `vault/vault.move`。
...
1. 借 `0`：触发 `EInvalidLoanQuantity = 3`。
...
4. 借 base 还 quote，或还款数量不是精确本金：分别触发 `EIncorrectTypeReturned = 5`、`EIncorrectQuantityReturned = 6`。
