...
- 确认 Predict 当前只能按源码快照和迁移状态阅读，不能默认视为稳定主网产品。
- 区分已合入的 Oracle/Vault/Configs 与仍处于迁移计划的 Indexer、Server、脚本服务。
- 建立后续阅读 `Predict`、`Registry`、`PredictManager`、Oracle、Vault、PLP 的版本边界。
